...
export type WorkflowView = 'history' | 'form' | 'confirm' | 'detail' | 'void';
...
export function workflowReducer(state: WorkflowState, action: WorkflowAction): WorkflowState {
  switch (action.type) {
    ...
    case 'START_CREATE': {
      const dose = action.suggestedDose !== undefined && action.suggestedDose > 0
    ...
    case 'STEP_DOSE': {
      ...
      if (!Number.isFinite(current) || current <= 0) {
        ...
        return { ...state, units: String(fallback), formError: null };
      }
...
export function useInjectionWorkflow() {
  ...
  const validateAndReview = useCallback(() => dispatch({ type: 'VALIDATE_AND_REVIEW' }), []);
